...
export const SystemOverviewCard: React.FC<SystemOverviewCardProps> = ({
  ...
  if (!overview) {
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">系统概览</h3>
        <div className="text-center text-gray-500">无法获取系统概览数据</div>
      </div>
    );
  }
        ...
        {stats.map((stat, index) => (
          <div key={index} className={`p-4 rounded-lg ${stat.color}`}>
            <div className="flex items-center justify-between mb-3">
              <div className={stat.color.split(' ')[0]}>{stat.icon}</div>
            </div>
        ...
        <div className="text-sm text-gray-600 space-y-2">
          ...
          {overview.collectionsCount > 0 && overview.documentsCount > 0 && (
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {(overview.documentsCount / overview.collectionsCount).toFixed(
                  1,
                )}
              </span>
            </div>
          ...
          {overview.chunksCount > 0 && overview.vectorsCount > 0 && (
            <div className="flex justify-between">
              <span>向量化覆盖率:</span>
              <span className="font-medium">
                {((overview.vectorsCount / overview.chunksCount) * 100).toFixed(
                  1,
                )}
                %
              </span>
