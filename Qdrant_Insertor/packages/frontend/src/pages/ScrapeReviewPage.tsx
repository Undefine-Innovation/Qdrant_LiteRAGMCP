  ...
  const load = async () => {
    ...
    try {
      const [res, cols, grp] = await Promise.all([
        scrapeApiService.getScrapeResults({
          status: 'PENDING',
          taskId: activeTaskId,
          limit,
          offset: page * limit,
      ...
      const list = Array.isArray(cols as unknown[])
        ? (cols as unknown[])
        : (cols as { data?: unknown[] }).data;
      setCollections((list as Collection[]) || []);
      setResults(res.items || []);
      setGroups(grp.groups || []);
      if (!selectedCollectionId && (list?.length ?? 0) > 0) {
        setSelectedCollectionId((list as Collection[])[0].collectionId);
      }
    } catch (e: unknown) {
            ...
            <ul className="space-y-2">
              {groups.map(g => (
                <li
                  ...
                  onClick={() => {
                    setActiveTaskId(g.taskId);
                    setPage(0);
                  }}
                >
                  <div className="text-sm font-medium truncate">{g.taskId}</div>
                  <div className="text-xs text-gray-500">
                    待审 {g.pending} / 共 {g.total}
                  </div>
                  <div className="text-xs text-gray-400">
                    {new Date(g.last_at).toLocaleString('zh-CN')}
                  </div>
                </li>
              ))}
            </ul>
            ...
            <div className="mb-6 p-4 bg-white border rounded">
              ...
              <div className="mt-3 space-x-2 flex flex-wrap items-center gap-2">
                <span className="text-xs text-gray-500 mr-2">
                  当前任务：
                  {activeTaskId ? (
                    <span className="text-gray-800">{activeTaskId}</span>
                  ) : (
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    setImportingId('task');
                    try {
                      const r = await scrapeApiService.importTask(
                        activeTaskId,
                        { collectionId: selectedCollectionId },
                      );
                      if (!r.success) alert('批量导入失败');
                      else await load();
                    } finally {
                      setImportingId(null);
                    }
                  ...
                  <div key={item.id} className="bg-white border rounded p-4">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        {item.snippet && (
                          <div className="mt-2 text-sm text-gray-600 line-clamp-3 max-w-3xl">
                            {item.snippet}
                          </div>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              const d = await scrapeApiService.getScrapeResult(
                                item.id,
                              );
                              if (d.success)
                                setPreviewItem(d.item as ScrapeResultItem);
                              else alert('加载详情失败');
                            } catch (e: unknown) {
                              const error = e as { message?: string };
