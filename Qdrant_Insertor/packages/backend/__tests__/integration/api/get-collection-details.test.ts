...
describe('GET /api/collections/:id - Collection Details Tests', () => {
  ...
  describe('成功场景', () => {
    ...
    it.skip('应该返回完整的集合信息', async () => {
      ...
      expect(collection.updated_at).toBeGreaterThanOrEqual(
  ...
  describe('错误场景', () => {
    ...
    it.skip('应该处理无效的UUID格式', async () => {
      const response = await request(testEnv.app).get(
        '/api/collections/invalid-uuid-format',
      );
    ...
    it.skip('应该处理SQL注入尝试', async () => {
      ...
      const response = await request(testEnv.app).get(
        `/api/collections/${encodeURIComponent(maliciousId)}`,
      );
  ...
  describe('响应格式验证', () => {
    ...
    it.skip('应该在data字段中包含集合信息', async () => {
      const response = await request(testEnv.app)
        .get(`/api/collections/${testCollection.id}`)
        .expect(200);
  ...
  describe('边界条件测试', () => {
    it.skip('应该处理极长的集合ID', async () => {
      const veryLongId = 'a'.repeat(1000);

      const response = await request(testEnv.app).get(
        `/api/collections/${veryLongId}`,
      );
    ...
    it.skip('应该处理特殊字符的集合ID', async () => {
      const specialCharId = '!@#$%^&*()';

      const response = await request(testEnv.app).get(
        `/api/collections/${encodeURIComponent(specialCharId)}`,
      );
    ...
    it.skip('应该处理Unicode字符的集合ID', async () => {
      const unicodeId = '测试集合ID';

      const response = await request(testEnv.app).get(
        `/api/collections/${encodeURIComponent(unicodeId)}`,
      );
