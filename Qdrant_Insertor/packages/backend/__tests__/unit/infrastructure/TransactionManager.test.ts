...
describe('TransactionManager', () => {
  ...
  beforeEach(() => {
    ...
    mockSqliteCore = {
      transaction: jest.fn(),
    } as unknown as jest.Mocked<SQLiteRepoCore>;
  ...
  describe('getTransactionStatus', () => {
    it('应该返回活跃事务的状态', async () => {
      const context = await transactionManager.beginTransaction();
      const status = transactionManager.getTransactionStatus(
  ...
  describe('executeInTransaction', () => {
    it('应该成功执行事务并提交', async () => {
      ...
      mockSqliteCore.transaction.mockImplementation((fn) =>
        Promise.resolve(fn()),
      );
    ...
    it('应该在函数抛出错误时回滚事务', async () => {
      ...
      expect(mockFn).toHaveBeenCalled();
      // 如果 rollback 失败时才记录错误日志
      // 由于我们的 mock 不会失败，所以 error 日志不会被调用
    });
  });

  describe('commit', () => {
    it('应该成功提交 PENDING 状态的事务', async () => {
      const context = await transactionManager.beginTransaction();
  ...
  describe('executeOperation', () => {
    it('应该在PENDING状态下执行操作并将事务设为ACTIVE', async () => {
      ...
      await transactionManager.executeOperation(
        context.transactionId,
        operation,
      );

      const updatedContext = transactionManager.getTransactionStatus(
        context.transactionId,
      );
    ...
    it('应该在ACTIVE状态下执行操作', async () => {
      ...
      expect(updatedContext?.operations[1]).toEqual(
    ...
    it('应该为已完成的事务抛出错误', async () => {
      ...
      // 手动设置事务状态为COMMITTED
      (
        transactionManager as unknown as Record<string, unknown>
      ).activeTransactions =
        (transactionManager as unknown as Record<string, unknown>)
          .activeTransactions || new Map();
      const transactions = (
        transactionManager as unknown as Record<string, unknown>
      ).activeTransactions as Map<string, unknown>;
      (
        transactions.get(context.transactionId) as Record<string, unknown>
      ...
      await expect(
        ...
        `Transaction ${context.transactionId} is not in a valid state for operations`,
      );
    });
  });
  ...
  describe('cleanupCompletedTransactions', () => {
    ...
    it('应该清理过期的已完成事务', async () => {
      ...
      // 手动设置事务状态
      const tm = transactionManager as unknown as Record<string, unknown>;
      const txns = tm.activeTransactions as Map<
        string,
        Record<string, unknown>
      >;
      txns.get(context1.transactionId)!.status = TransactionStatus.COMMITTED;
      txns.get(context2.transactionId)!.status = TransactionStatus.ACTIVE;
    ...
    it('应该使用自定义的最大保留时间', async () => {
      ...
      const txns = tm.activeTransactions as Map<
        ...
        Record<string, unknown>
      >;
      txns.get(context.transactionId)!.status = TransactionStatus.COMMITTED;
  ...
  describe('deleteCollectionInTransaction', () => {
    it('应该在事务中删除集合', async () => {
      ...
      expect(
        mockCollections.chunkMeta.deleteByCollectionId,
      ).toHaveBeenCalledWith(collectionId);
      expect(
        mockCollections.chunksFts5.deleteByCollectionId,
      ).toHaveBeenCalledWith(collectionId);
      expect(mockCollections.docs.listByCollection).toHaveBeenCalledWith(
        collectionId,
      );
