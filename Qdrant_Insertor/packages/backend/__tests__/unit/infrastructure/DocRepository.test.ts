...
describe('DocRepository', () => {
  ...
  describe('findWithPagination', () => {
    it('应该返回分页的文档', async () => {
      ...
      // Act
      const result = await docRepository.findWithPagination(
        paginationOptions,
        collectionId,
        status,
      );
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      const superFindWithPagination = jest.spyOn(
        Object.getPrototypeOf(Object.getPrototypeOf(docRepository)),
        'findWithPagination',
      );
      ...
      // Act & Assert
      await expect(
        docRepository.findWithPagination(paginationOptions),
      ).rejects.toThrow('Database error');
  ...
  describe('batchSoftDelete', () => {
    it('应该批量软删除文档', async () => {
      ...
      jest
        .spyOn(docRepository, 'softDeleteBatch')
  ...
  describe('restore', () => {
    it('应该恢复已删除的文档', async () => {
      ...
      expect(mockRepository.update).toHaveBeenCalledWith(docId, {
        deleted: false,
        deleted_at: expect.any(Function),
        updated_at: expect.any(Number),
      });
    ...
    it('当数据库操作失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(docRepository.restore(docId)).rejects.toThrow(
        'Database error',
      );
  ...
  describe('findByContentHash', () => {
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(
        docRepository.findByContentHash(contentHash),
      ).rejects.toThrow('Database error');
      expect(mockLogger.error).toHaveBeenCalledWith(
        '根据内容哈希查找文档失败',
        {
  ...
  describe('batchUpdateStatus', () => {
    it('应该批量更新文档状态', async () => {
      ...
      jest
        .spyOn(docRepository, 'updateBatch')
      ...
      expect(docRepository.updateBatch).toHaveBeenCalledWith(docIds, {
        status,
      });
  ...
  describe('deleteWithManager', () => {
    ...
    it('当删除失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(
        docRepository.deleteWithManager(docId, mockManager),
