...
import { CollectionRepository } from '../../../../src/infrastructure/database/repositories/CollectionRepository.js';
...
describe('CollectionRepository', () => {
  ...
  describe('findByName', () => {
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.findByName(collectionName)).rejects.toThrow('Database error');
  ...
  describe('findByCollectionId', () => {
    it('应该根据集合ID找到集合', async () => {
      ...
      // Act
      const result = await collectionRepository.findByCollectionId(collectionId);
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.findByCollectionId(collectionId)).rejects.toThrow('Database error');
  ...
  describe('findAllActive', () => {
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.findAllActive()).rejects.toThrow('Database error');
  ...
  describe('findWithPagination', () => {
    it('应该返回分页的集合', async () => {
      ...
      // Act
      const result = await collectionRepository.findWithPagination(paginationOptions);
    ...
    it('应该支持状态过滤', async () => {
      ...
      // Assert
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('collection.status = :status', { status });
    });
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.findWithPagination(paginationOptions)).rejects.toThrow('Database error');
  ...
  describe('updateCollection', () => {
    it('应该更新集合信息', async () => {
      ...
      jest.spyOn(collectionRepository, 'update').mockResolvedValue(updatedCollection);

      // Act
      const result = await collectionRepository.updateCollection(collectionId, updateData);

      // Assert
      expect(result).toEqual(updatedCollection);
      expect(collectionRepository.update).toHaveBeenCalledWith(collectionId, updateData);
    ...
    it('当更新失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.updateCollection(collectionId, updateData)).rejects.toThrow('Update failed');
  ...
  describe('existsByName', () => {
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.existsByName(collectionName)).rejects.toThrow('Database error');
  ...
  describe('getCount', () => {
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.getCount()).rejects.toThrow('Database error');
  ...
  describe('findByPrefix', () => {
    it('应该根据前缀查找集合', async () => {
      ...
      // Assert
      expect(result).toEqual(expectedCollections);
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('collection.name LIKE :prefix', { prefix: `${prefix}%` });
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('collection.deleted = :deleted', { deleted: false });
      expect(mockQueryBuilder.orderBy).toHaveBeenCalledWith('collection.created_at', 'DESC');
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.findByPrefix(prefix)).rejects.toThrow('Database error');
  ...
  describe('batchUpdateStatus', () => {
    it('应该批量更新集合状态', async () => {
      ...
      jest.spyOn(collectionRepository, 'updateBatch').mockResolvedValue(expectedResult);

      // Act
      const result = await collectionRepository.batchUpdateStatus(collectionIds, status);

      // Assert
      expect(result).toEqual({ updated: 2, failed: 0 });
      expect(collectionRepository.updateBatch).toHaveBeenCalledWith(collectionIds, { status });
    ...
    it('当批量更新失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.batchUpdateStatus(collectionIds, status)).rejects.toThrow('Batch update failed');
  ...
  describe('batchSoftDelete', () => {
    it('应该批量软删除集合', async () => {
      ...
      jest.spyOn(collectionRepository, 'softDeleteBatch').mockResolvedValue(deletedCount);
      ...
      expect(collectionRepository.softDeleteBatch).toHaveBeenCalledWith(collectionIds);
    ...
    it('当批量软删除失败时应该抛出错误', async () => {
      ...
      jest.spyOn(collectionRepository, 'softDeleteBatch').mockRejectedValue(error);

      // Act & Assert
      await expect(collectionRepository.batchSoftDelete(collectionIds)).rejects.toThrow('Batch soft delete failed');
  ...
  describe('updateDocumentCount', () => {
    it('应该更新集合文档数量', async () => {
      ...
      // Act
      const result = await collectionRepository.updateDocumentCount(collectionId, count);

      // Assert
      expect(result).toBe(true);
      expect(collectionRepository.update).toHaveBeenCalledWith(collectionId, { documentCount: count });
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.updateDocumentCount(collectionId, count)).rejects.toThrow('Update failed');
  ...
  describe('getStatistics', () => {
    it('应该返回集合统计信息', async () => {
      ...
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(CASE WHEN status = :active THEN 1 ELSE 0 END)', 'active');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(CASE WHEN status = :inactive THEN 1 ELSE 0 END)', 'inactive');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(CASE WHEN status = :archived THEN 1 ELSE 0 END)', 'archived');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(documentCount)', 'totalDocuments');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(chunkCount)', 'totalChunks');
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('collection.deleted = :deleted', { deleted: false });
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(collectionRepository.getStatistics()).rejects.toThrow('Database error');
