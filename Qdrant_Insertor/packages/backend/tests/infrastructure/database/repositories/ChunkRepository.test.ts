...
import { ChunkRepository } from '../../../../src/infrastructure/database/repositories/ChunkRepository.js';
...
describe('ChunkRepository', () => {
  ...
  describe('findByDocId', () => {
    it('应该根据文档ID查找块', async () => {
      ...
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('chunk.docId = :docId', { docId });
      expect(mockQueryBuilder.addOrderBy).toHaveBeenCalledWith('chunk.chunkIndex', 'ASC');
      expect(mockQueryBuilder.select).toHaveBeenCalledWith([
        'chunk.id', 'chunk.pointId', 'chunk.docId', 'chunk.collectionId', 
        'chunk.chunkIndex', 'chunk.title', 'chunk.contentLength', 'chunk.tokenCount', 
        'chunk.embeddingStatus', 'chunk.syncStatus', 'chunk.embeddedAt', 'chunk.syncedAt', 
        'chunk.error', 'chunk.metadata', 'chunk.created_at', 'chunk.updated_at'
      ]);
    });
    ...
    it('应该支持状态过滤', async () => {
      ...
      // Assert
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('chunk.embeddingStatus = :status', { status });
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('chunk.syncStatus = :syncStatus', { syncStatus });
    });
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.findByDocId(docId)).rejects.toThrow('Database error');
  ...
  describe('findByCollectionId', () => {
    it('应该根据集合ID查找块', async () => {
      ...
      // Assert
      expect(result).toEqual(expectedChunks);
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('chunk.collectionId = :collectionId', { collectionId });
      expect(mockQueryBuilder.addOrderBy).toHaveBeenCalledWith('chunk.chunkIndex', 'ASC');
    });
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.findByCollectionId(collectionId)).rejects.toThrow('Database error');
  ...
  describe('findByPointIds', () => {
    it('应该根据点ID数组查找块', async () => {
      ...
      // Assert
      expect(result).toEqual(expectedChunks);
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('chunk.pointId IN (:...pointIds)', { pointIds });
      expect(mockQueryBuilder.addOrderBy).toHaveBeenCalledWith('chunk.chunkIndex', 'ASC');
    });
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.findByPointIds(pointIds)).rejects.toThrow('Database error');
  ...
  describe('findByPointId', () => {
    it('应该根据点ID查找单个块', async () => {
      ...
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('chunk.pointId = :pointId', { pointId });
      expect(mockQueryBuilder.select).toHaveBeenCalledWith([
        'chunk.id', 'chunk.pointId', 'chunk.docId', 'chunk.collectionId', 
        'chunk.chunkIndex', 'chunk.title', 'chunk.contentLength', 'chunk.tokenCount', 
        'chunk.embeddingStatus', 'chunk.syncStatus', 'chunk.embeddedAt', 'chunk.syncedAt', 
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.findByPointId(pointId)).rejects.toThrow('Database error');
  ...
  describe('createBatch', () => {
    it('应该批量创建块', async () => {
      ...
      const superCreateBatch = jest.spyOn(Object.getPrototypeOf(Object.getPrototypeOf(chunkRepository)), 'createBatch');
    ...
    it('应该支持自定义批次大小', async () => {
      ...
      jest.spyOn(chunkRepository, 'createBatch' as any).mockResolvedValue(expectedChunks);
    ...
    it('当批量创建失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.createBatch(chunks)).rejects.toThrow('Batch create failed');
  ...
  describe('deleteByDocId', () => {
    ...
    it('当删除失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.deleteByDocId(docId)).rejects.toThrow('Delete failed');
  ...
  describe('deleteByCollectionId', () => {
    ...
    it('当删除失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.deleteByCollectionId(collectionId)).rejects.toThrow('Delete failed');
  ...
  describe('deleteByPointIds', () => {
    it('应该根据点ID数组批量删除块', async () => {
      ...
      // Act
      const result = await chunkRepository.deleteByPointIds(pointIds, batchSize);
    ...
    it('当删除失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.deleteByPointIds(pointIds)).rejects.toThrow('Delete failed');
  ...
  describe('getCount', () => {
    ...
    it('应该支持文档ID过滤', async () => {
      ...
      // Assert
      expect(result).toBe(expectedCount);
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('chunk.docId = :docId', { docId });
    });
    ...
    it('应该支持集合ID过滤', async () => {
      ...
      // Assert
      expect(result).toBe(expectedCount);
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('chunk.collectionId = :collectionId', { collectionId });
    });
    ...
    it('应该支持状态过滤', async () => {
      ...
      // Act
      const result = await chunkRepository.getCount(undefined, undefined, { status, syncStatus });
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.getCount()).rejects.toThrow('Database error');
  ...
  describe('batchUpdateStatus', () => {
    it('应该批量更新块状态', async () => {
      ...
      jest.spyOn(chunkRepository, 'updateBatch').mockResolvedValue(expectedResult);
      ...
      expect(chunkRepository.updateBatch).toHaveBeenCalledWith(pointIds, { embeddingStatus: status });
    ...
    it('当批量更新失败时应该抛出错误', async () => {
      ...
      jest.spyOn(chunkRepository, 'updateBatch').mockRejectedValue(error);

      // Act & Assert
      await expect(chunkRepository.batchUpdateStatus(pointIds, status)).rejects.toThrow('Batch update failed');
  ...
  describe('batchUpdateSyncStatus', () => {
    it('应该批量更新块同步状态', async () => {
      ...
      // Act
      const result = await chunkRepository.batchUpdateSyncStatus(pointIds, syncStatus);

      // Assert
      expect(result).toEqual(expectedResult);
      expect(chunkRepository.updateBatch).toHaveBeenCalledWith(pointIds, { syncStatus });
    ...
    it('当批量更新失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.batchUpdateSyncStatus(pointIds, syncStatus)).rejects.toThrow('Batch update failed');
  ...
  describe('getStatistics', () => {
    it('应该返回块统计信息', async () => {
      ...
      // Act
      const result = await chunkRepository.getStatistics({ collectionId, docId });

      // Assert
      expect(result).toEqual(expectedStats);
      expect(mockQueryBuilder.select).toHaveBeenCalledWith('COUNT(*)', 'total');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(CASE WHEN embeddingStatus = :pending THEN 1 ELSE 0 END)', 'pending');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(CASE WHEN embeddingStatus = :processing THEN 1 ELSE 0 END)', 'processing');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(CASE WHEN embeddingStatus = :completed THEN 1 ELSE 0 END)', 'completed');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(CASE WHEN embeddingStatus = :failed THEN 1 ELSE 0 END)', 'failed');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('SUM(tokenCount)', 'totalTokens');
      expect(mockQueryBuilder.addSelect).toHaveBeenCalledWith('AVG(contentLength)', 'avgContentLength');
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('chunk.collectionId = :collectionId', { collectionId });
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('chunk.docId = :docId', { docId });
    ...
    it('当数据库查询失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.getStatistics({ collectionId })).rejects.toThrow('Database error');
  ...
  describe('createBatchWithManager', () => {
    it('应该使用事务管理器批量创建块', async () => {
      ...
      // Act
      const result = await chunkRepository.createBatchWithManager(chunks, mockManager);
    ...
    it('当批量创建失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.createBatchWithManager(chunks, mockManager)).rejects.toThrow('Batch create failed');
      expect(mockLogger.error).toHaveBeenCalledWith('批量创建块失败', {
        count: chunks.length,
        error: 'Batch create failed',
      });
  ...
  describe('deleteByDocIdWithManager', () => {
    it('应该使用事务管理器根据文档ID删除块', async () => {
      ...
      // Act
      const result = await chunkRepository.deleteByDocIdWithManager(docId, mockManager);
    ...
    it('当删除失败时应该抛出错误', async () => {
      ...
      // Act & Assert
      await expect(chunkRepository.deleteByDocIdWithManager(docId, mockManager)).rejects.toThrow('Delete failed');
      expect(mockLogger.error).toHaveBeenCalledWith('根据文档ID删除块失败', {
        docId,
        error: 'Delete failed',
      });
