...
describe('TransactionManager', () => {
  ...
  describe('commit', () => {
    ...
    it('应该为非活跃事务抛出错误', async () => {
      ...
      ).rejects.toThrow(`Transaction ${context.transactionId} is not active`);
    });
  });
  ...
  describe('cleanupCompletedTransactions', () => {
    ...
    it('应该清理过期的已完成事务', () => {
      ...
      // 手动设置事务状态
      (transactionManager as any).activeTransactions.get(
        context1.transactionId,
      ).status = TransactionStatus.COMMITTED;
      (transactionManager as any).activeTransactions.get(
        context2.transactionId,
  ...
  describe('deleteCollectionInTransaction', () => {
    it('应该在事务中删除集合', async () => {
      ...
      ).deleteCollectionInTransaction(collectionId, mockCollections as any);
