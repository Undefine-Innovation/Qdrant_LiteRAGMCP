...
describe('Collection Entity Integration Tests', () => {
  ...
  describe('Collection Creation', () => {
    ...
    it('应该拒绝创建空名称的集合', async () => {
      ...
      // Act & Assert
      await expect(repository.save(collection)).rejects.toThrow('Collection name cannot be empty');
    });
  });
  ...
  describe('Collection Deletion', () => {
    ...
    it('删除集合时应该级联删除相关文档', async () => {
      ...
      // Act - 手动删除相关文档以模拟级联删除
      await docRepository.delete({ collectionId: savedCollection.collectionId });
  ...
  describe('Collection Constraints', () => {
    ...
    it('应该强制名称长度约束', async () => {
      ...
      // Act & Assert
      await expect(repository.save(collection)).rejects.toThrow('Collection name cannot exceed 255 characters');
    });
  });
