...
describe('Batch API E2E Tests', () => {
  ...
  describe('POST /api/batch/upload', () => {
    it('应该成功批量上传文档', async () => {
      ...
      // Assert
      ApiTestUtils.validateApiResponse(result, 200, ['success', 'total', 'successful', 'failed', 'results']);
    ...
    it('应该处理部分失败的批量上传', async () => {
      ...
      const invalidFile = ApiTestUtils.createTestFile('invalid.exe', 'executable content', 'application/octet-stream');
  ...
  describe('DELETE /api/docs/batch', () => {
    it('应该成功批量删除文档', async () => {
      ...
      // Assert
      ApiTestUtils.validateApiResponse(response, 200, ['success', 'total', 'successful', 'failed', 'results']);
  ...
  describe('GET /api/batch/progress/:operationId', () => {
    it('应该返回批量操作进度', async () => {
      ...
      if (response.status === 200) {
        ApiTestUtils.validateApiResponse(response, 200, ['operationId', 'type', 'status', 'total', 'processed']);
    ...
    it('应该返回404当操作ID不存在', async () => {
      // Act
      const response = await ApiTestUtils.createRequest(testEnv.app)
        .get('/api/batch/progress/non-existent-operation');
  ...
  describe('Error Handling', () => {
    ...
    it('应该处理数据库连接错误', async () => {
      ...
      // Assert
      ApiTestUtils.validateErrorResponse(response, 500, 'INTERNAL_SERVER_ERROR');
    });
  });
