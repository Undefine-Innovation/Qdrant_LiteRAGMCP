...
describe('Search API E2E Tests', () => {
    ...
    const chunksData = [
      {
        ...
        content: 'Artificial intelligence is a rapidly evolving field that encompasses machine learning, neural networks, and deep learning technologies.',
        contentLength: 147,
      },
      {
        docId: testDocuments[0].id,
        ...
        content: 'Machine learning algorithms enable computers to learn from data without being explicitly programmed.',
        contentLength: 103,
      },
      {
        ...
        chunkIndex: 0,
        title: 'HTML and CSS',
        content: 'HTML provides structure of web pages while CSS handles presentation and styling.',
        contentLength: 89,
      },
      {
        docId: testDocuments[1].id,
        ...
        content: 'Modern JavaScript frameworks like React, Vue, and Angular simplify complex web application development.',
        contentLength: 108,
      },
      {
  ...
  describe('POST /api/search', () => {
    it('应该执行搜索并返回结果', async () => {
      ...
      // Assert
      ApiTestUtils.validateApiResponse(response, 200, ['results', 'total', 'query']);
  ...
  describe('Search Performance', () => {
    ...
    it('应该处理大量搜索请求', async () => {
      // Arrange
      const searchQueries = Array(10).fill(null).map((_, i) =>
      ...
      const promises = searchQueries.map(query =>
        ApiTestUtils.createRequest(testEnv.app)
          .post('/api/search')
          .send(query)
      );
