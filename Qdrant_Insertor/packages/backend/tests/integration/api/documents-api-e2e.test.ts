...
describe('Documents API E2E Tests', () => {
  ...
  async function createTestData() {
    try {
      ...
      const now = Date.now();
      const collectionId = 'test-collection-' + Math.random().toString(36).substring(2, 8);
      ...
      await testEnv.dataSource.query(
        `INSERT INTO collections (id, collectionId, name, description, status, created_at, updated_at, deleted, version) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [collectionId, collectionId, collectionName, 'Test collection', 'active', now, now, false, 1]
      );
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        await testEnv.dataSource.query(
          `INSERT INTO docs (id, docId, collectionId, key, name, content, size_bytes, status, created_at, updated_at, deleted, version) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [docId, docId, collectionId, `key-${i}`, docName, docContent, docContent.length, 'new', now, now, false, 1]
        );
  ...
  describe('POST /api/documents', () => {
    it('应该成功上传文档到指定集合', async () => {
      ...
      // Act
      console.log('[Test] 发送POST请求到:', `/api/collections/${testCollection.id}/docs`);
      ...
      // Assert
      ApiTestUtils.validateApiResponse(response, 201, ['docId', 'name', 'collectionId']);
    ...
    it('应该拒绝不支持的文件类型', async () => {
      ...
      // Assert
      ApiTestUtils.validateErrorResponse(response, 422, 'UNSUPPORTED_FILE_TYPE');
    });
  ...
  describe('GET /api/documents', () => {
    ...
    it('应该支持按集合过滤', async () => {
      ...
      // Assert
      ApiTestUtils.validatePaginatedResponse(response, 3);
      expect(response.body.data.every((doc: any) => doc.collectionId === testCollection.id)).toBe(true);
    });
  ...
  describe('GET /api/documents/:docId', () => {
    it('应该返回指定的文档', async () => {
      // Act
      const response = await ApiTestUtils.createRequest(testEnv.app)
        .get(`/api/docs/${testDocuments[0].id}`);

      // Assert
      ApiTestUtils.validateApiResponse(response, 200, ['id', 'name', 'content', 'collectionId']);
  ...
  describe('PUT /api/documents/:docId/resync', () => {
    it('应该成功重新同步文档', async () => {
      // Act
      const response = await ApiTestUtils.createRequest(testEnv.app)
        .put(`/api/docs/${testDocuments[0].id}/resync`);
  ...
  describe('DELETE /api/documents/:docId', () => {
    it('应该成功删除文档', async () => {
      // Act
      const response = await ApiTestUtils.createRequest(testEnv.app)
        .delete(`/api/docs/${testDocuments[0].id}`);
  ...
  describe('GET /api/documents/:docId/chunks', () => {
    ...
    it('应该返回404当文档不存在', async () => {
      // Act
      const response = await ApiTestUtils.createRequest(testEnv.app)
        .get('/api/docs/non-existent-id/chunks');
