...
describe('GET /api/documents - Document List Tests', () => {
  ...
  describe('过滤功能', () => {
    it('应该支持按集合过滤', async () => {
      ...
      // Assert
      expect(response.body.data).toHaveLength(5);
      expect(response.body.data.every((doc: any) => 
        doc.collectionId === testCollection.collectionId
      )).toBe(true);
    });
    ...
    it('应该支持按状态过滤', async () => {
      ...
      // Assert
      expect(response.body.data.every((doc: any) => doc.status === 'new')).toBe(true);
    });
    ...
    it('应该支持按名称搜索', async () => {
      ...
      expect(response.body.data.some((doc: any) => 
        doc.name.includes('Document 1')
      )).toBe(true);
    });
