...
import { createApiTestEnvironment, resetTestDatabase } from '../../integration/api/api-test-setup.test.js';

describe('Golden path e2e - Documents (integration copy)', () => {
  ...
  afterAll(async () => {
    if (env?.dataSource && env.dataSource.isInitialized) await env.dataSource.destroy();
  });
  ...
  it('happy path: upload a file to an existing collection and get it back', async () => {
    // create a collection
    const collResp = await request(env.app).post('/api/collections').send({
    ...
    const getResp = await request(env.app).get(`/api/docs/${uploadResp.body.docId}`).expect(200);
  ...
  it('failure path: uploading to non-existent collection returns current behavior', async () => {
