...
/**
 * 计算重试成功率
 * @param {number} successfulRetries - 成功重试次数
 * @param {number} totalRetries - 总重试次数
 * @returns {number} 成功率（0-1之间）
 */
...
/**
 * 计算平均重试时间
 * @param {number[]} retryTimes - 重试时间数组
 * @returns {number} 平均重试时间（毫秒）
 */
 ...
 * 更新重试时间数组，保持最大长度
 * @param {number[]} retryTimes - 重试时间数组
 * @param {number} newTime - 新的重试时间
 * @param {number} [maxLength=1000] - 最大长度
 * @returns {number[]} 更新后的重试时间数组
 */
