...
export function createDocumentRoutes(
   ...
   * @apiGroup Documents
   * @apiDescription 根据文档ID获取该文档的所有块。
   * @apiParam {string} docId - 文档的唯一标识符。
   * @apiSuccess {Object[]} chunks - 文档块数组。
   * @apiSuccess {string} chunks.pointId - 块的点ID。
   * @apiSuccess {string} chunks.docId - 文档ID。
   * @apiSuccess {string} chunks.collectionId - 集合ID。
   * @apiSuccess {number} chunks.chunkIndex - 块索引。
   * @apiSuccess {string} chunks.title - 块标题（可选）。
   * @apiSuccess {string} chunks.content - 块内容。
   * @apiSuccessExample {json} Success-Response:
   *     HTTP/1.1 200 OK
   *     [
   *       {
