...
export const ScrapeStartRequestSchema = z.object({
  ...
  }).optional().describe('CSS选择器配置'),
  headers: z.record(z.string(), z.string()).optional().describe('自定义请求头'),
  timeout: z.number().int().min(1000).max(300000).optional().describe('请求超时时间（毫秒）'),
...
export const ScrapeStatusResponseSchema = z.object({
  ...
  task: z.object({
    ...
    context: z.record(z.string(), z.unknown()).optional().describe('任务上下文'),
  }).describe('任务状态信息'),
});
...
export const ScrapeStatsResponseSchema = z.object({
  success: z.boolean().describe('查询是否成功'),
  stats: z.record(z.string(), z.record(z.string(), z.number())).describe('爬虫任务统计信息'),
});
