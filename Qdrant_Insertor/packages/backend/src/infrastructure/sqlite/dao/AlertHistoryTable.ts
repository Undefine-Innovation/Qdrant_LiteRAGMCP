...
export class AlertHistoryTable {
  ...
  update(
    id: string,
    updates: Partial<Omit<AlertHistory, 'id' | 'createdAt' | 'triggeredAt'>>,
  ): boolean {
    const fields = [];
    const values = [];
  ...
  getByRuleId(ruleId: string, limit?: number): AlertHistory[] {
    let query =
      'SELECT * FROM alert_history WHERE rule_id = ? ORDER BY triggered_at DESC';
    const params = [ruleId];
  ...
  getByStatus(status: string, limit?: number): AlertHistory[] {
    let query =
      'SELECT * FROM alert_history WHERE status = ? ORDER BY triggered_at DESC';
    const params = [status];
