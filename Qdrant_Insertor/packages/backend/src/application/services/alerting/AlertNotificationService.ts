...
export class AlertNotificationService {
  ...
  private async sendWebhookNotification(
    ...
    // 这里应该实现实际的HTTP请求
    // 暂时只记录日志
    this.logger.info(`[Webhook] 发送告警通知`, {
  ...
  /**
   * 获取通知渠道列表
   * @returns Promise<NotificationChannel[]> - 返回通知渠道列表的 Promise
   */
   ...
   * @returns Promise<NotificationChannel | null> - 返回通知渠道或 null 的 Promise
   */
   ...
   * @param message - 通知消息
   * @param severity - 严重程度
   * @returns Promise<{ success: boolean; message: string; timestamp: string }> - 测试结果
   */
