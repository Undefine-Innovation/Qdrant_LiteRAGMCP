...
export class ScrapeStateMachineStrategy extends BaseStateMachineStrategy {
  ...
  private async appendLog(
    ...
    const logs = Array.isArray((ctx as any).logs) ? ((ctx as any).logs as any[]) : [];
    const record =
      typeof entry === 'string'
        ? { ts: Date.now(), level: 'info', message: entry }
        : { ts: Date.now(), level: entry.level ?? 'info', message: entry.message };
    logs.push(record);
  ...
  async executeTask(taskId: string): Promise<void> {
    ...
    try {
      ...
      await this.appendLog(taskId, `开始爬取: ${(context.config as ScrapeConfig).url}`);
      ...
      if (result.status === 'COMPLETED') {
        ...
        await this.appendLog(taskId, `爬取完成，解析到 ${Array.isArray((result as any).links) ? (result as any).links.length : 0} 个链接`);
        await this.handleTransition(taskId, BaseEvent.COMPLETE, {
          ...
          links: result.links,
          pages: (result.metadata as any)?.pages,
        });
        // 结束时标记为100%
        await this.persistence.updateTask(taskId, { progress: 100 });
        await this.appendLog(taskId, '任务完成 ✅');
      } else {
        await this.appendLog(taskId, { level: 'error', message: `爬取失败: ${(result as any).error ?? '未知错误'}` });
      ...
      await this.appendLog(taskId, { level: 'error', message: `执行异常: ${(error as Error).message}` });
      await this.handleTransition(taskId, BaseEvent.FAIL, {
        ...context,
...
export class ScrapeService implements IScrapeService {
  ...
  this.importService = importService;
    ...
    // 获取状态机引擎的持久化实例
    const persistence = (stateMachine as any).persistence as StatePersistence;
    if (!persistence) {
          ...
          },
          {
            ...
            action: async (context: Record<string, unknown>) => {
              ...
              try {
                ...
                const content = (context as any).content as string | undefined;
                const links = (context as any).links as Array<{ url: string; text?: string; title?: string }> | undefined;
                const pages = (context as any).pages as Array<{ url: string; title?: string; content?: string }> | undefined;
                if (!this.sqliteRepo) {
                  this.logger.warn('ScrapeService 未注入 sqliteRepo，跳过结果持久化');
                  return;
                }
                if (!taskId || !cfg?.url) {
  ...
  > {
    ...
    const records = this.sqliteRepo.scrapeResults.list({
  ...
  > {
    ...
    const rec = this.sqliteRepo.scrapeResults.getById(id);
    return rec ?? null;
  }

  /** 按任务分组聚合 */
  async listScrapeTaskGroups(params?: { limit?: number; offset?: number }): Promise<
    Array<{ taskId: string; total: number; pending: number; imported: number; deleted: number; first_at: number; last_at: number }>
  > {
    if (!this.sqliteRepo) return [];
    return this.sqliteRepo.scrapeResults.getTaskGroups({ limit: params?.limit, offset: params?.offset });
  }
  ...
  async importScrapeResult(
    ...
    name?: string,
  ): Promise<{ success: boolean; docId?: import('@domain/entities/types.js').DocId; error?: string }> {
    ...
    const doc = await this.importService.importText(docName, content, collectionId);
    this.sqliteRepo.scrapeResults.markImported(id, doc.docId);
  ...
  async deleteScrapeResult(id: string): Promise<{ success: boolean }> {
    ...
    this.sqliteRepo.scrapeResults.delete(id);
    return { success: true };
  }
  ...
  async importTaskResults(
    ...
    collectionId: import('@domain/entities/types.js').CollectionId,
    namePrefix?: string,
  ): Promise<{ success: boolean; imported: number; errors?: Array<{ id: string; error: string }> }> {
    if (!this.sqliteRepo || !this.importService) {
      return { success: false, imported: 0, errors: [{ id: '*', error: 'Service not ready' }] };
    }
    const items = this.sqliteRepo.scrapeResults.list({ taskId, status: 'PENDING', includeContent: true });
    let imported = 0;
    const errors: Array<{ id: string; error: string }> = [];
    for (const it of items) {
      try {
        const name = (namePrefix ? `${namePrefix}-` : '') + (it.title || new URL(it.url).pathname.replace(/\/+/, '/'));
        const content = it.content || '';
        if (!content) {
          errors.push({ id: it.id, error: 'Empty content' });
          continue;
        }
        const doc = await this.importService.importText(name, content, collectionId);
        this.sqliteRepo.scrapeResults.markImported(it.id, doc.docId);
        imported++;
      } catch (e: any) {
        errors.push({ id: it.id, error: e?.message || 'Import failed' });
      }
    }
  ...
  async deleteTaskResults(taskId: string): Promise<{ success: boolean }> {
    if (!this.sqliteRepo) return { success: false };
    this.sqliteRepo.scrapeResults.deleteByTask(taskId);
    return { success: true };
  }
}
