...
/**
 * 管理文档同步的状态机，负责协调文档的分割、嵌入生成和 Qdrant 同步
 */
export class SyncStateMachine {
  ...
  /**
   * 构造函数
   * @param sqliteRepo - SQLite 仓库实例，用于访问SQLite 数据库
   * @param qdrantRepo - Qdrant 仓库实例，用于与 Qdrant 向量数据库交互
   * @param embeddingProvider - 嵌入提供者实例，用于生成文档内容的嵌入向量
   * @param splitter - 文档分割器实例，用于将文档内容分割成块
   * @param logger - 日志记录器实例，用于记录同步过程中的信息和错误
   */
  constructor(
  ...
  /**
   * 触发指定文档的同步过程。包括文档分割、嵌入生成和 Qdrant 向量数据库的更新
   * @param docId - 需要同步的文档 ID
   * @returns 无返回值
   */
