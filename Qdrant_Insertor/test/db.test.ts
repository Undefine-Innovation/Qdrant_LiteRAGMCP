...
describe('ChunksDatabase', () => {
  ...
  describe('Collection Operations', () => {
    test('should create and retrieve a collection', () => {
      ...
      const retrievedCollection = db.getCollectionById(
      ...
      expect(retrievedCollection?.collectionId).toBe(
        newCollection.collectionId,
      );
    });
  ...
  describe('Version Operations', () => {
    test('should create and retrieve a version', () => {
      const newVersion = db.createVersion(
        collectionId,
        'v2.0',
        'Second version',
      );
    ...
    test('should list versions for a collection', () => {
      ...
      expect(versions.some((v: Version) => v.versionId === versionId)).toBe(
        true,
      );
    });
  ...
  describe('Doc Operations', () => {
    ...
    test('should update document content without changing docId if content is same', () => {
      ...
      const newUpdatedDoc = db.updateDoc(
        originalDocId,
        newContent,
        'New Name',
        'application/json',
    ...
    test('should get all documents across all versions and collections', () => {
      ...
      const version2 = db.createVersion(
        collection2.collectionId,
        'v1.0',
        'Version for Collection Two',
      );
    ...
    test('should return null for non-existent chunk meta', () => {
      const validDocId = makeDocId('nonexistent content');
      const nonExistentPointId = makePointId(validDocId, 0);
      expect(db.getChunkMeta(nonExistentPointId)).toBeNull();
    });

    test('should return empty object for non-existent chunk texts', () => {
      const validDocId = makeDocId('non-existent content');
      const nonExistentPointId = makePointId(validDocId, 99);
        ...
        metas: [
          {
            pointId,
            chunkIndex: 0,
            titleChain: 'Single Chunk',
            contentHash: makeDocId('Single Content'),
          },
        ],
        ...
        metas: [
          {
            ...
            chunkIndex: 0,
            titleChain: 'Text A',
            contentHash: makeDocId('Content A'),
          },
          {
            pointId: pointId2,
            chunkIndex: 1,
            titleChain: 'Text B',
            contentHash: makeDocId('Content B'),
          },
        ],
        texts: [
  ...
  describe('Search Operations', () => {
        ...
        metas: [
          {
            ...
            contentHash: makeDocId('This is the content of the first chunk.'),
          },
          {
            pointId: pointId2,
            chunkIndex: 1,
            titleChain: 'Second Chunk Title',
            contentHash: makeDocId('Another chunk with different content.'),
          },
        ],
        texts: [
          {
    ...
    test('should filter keyword search results by additional filters', () => {
      ...
      const filteredDocContent =
        'This is content for a filtered document. Specific keyword.';
      const filteredDoc = db.createDoc(
        versionId,
        collectionId,
        'filtered-doc-key',
        filteredDocContent,
      ...
      db.insertChunkBatch({
        ...
        metas: [
          {
            pointId: filteredPointId,
            chunkIndex: 0,
            titleChain: 'Filtered Doc Title',
            contentHash: makeDocId(filteredDocContent),
          },
        ],
        texts: [
          {
            pointId: filteredPointId,
            content: filteredDocContent,
            title: 'Filtered Doc',
          },
        ],
      });
      ...
      test('should retrieve existing chunks and ignore non-existent ones', () => {
          ...
          metas: [
            {
              ...
              chunkIndex: 0,
              titleChain: 'Existing Chunk',
              contentHash: makeDocId('Existing Content'),
            },
          ],
          texts: [
            {
        ...
        const chunks = db.getChunksByPointIds(
          [existingPointId, nonExistentPointId],
          collectionId,
        );
        expect(chunks.length).toBe(1);
      ...
      expect(results.length).toBeGreaterThan(0);
      expect(
        results.every((r: SearchResult) => r.docId === filteredDoc.docId),
      ).toBe(true);
    ...
    test('should filter keyword search by latestOnly', () => {
      ...
      db.setCurrentVersion(newVersion.versionId, collectionId);
      const newDocContent =
        'This is new content for a new document. Contains searchable keyword.';
      const newDoc = db.createDoc(
        ...
        metas: [
          {
            pointId: newPointId,
            chunkIndex: 0,
            titleChain: 'New Content Title',
            contentHash: makeDocId(newDocContent),
          },
        ],
        texts: [
          { pointId: newPointId, content: newDocContent, title: 'New Content' },
        ],
      });
      ...
      expect(results.some((r: SearchResult) => r.docId === newDoc.docId)).toBe(
        true,
      );
      expect(results.some((r: SearchResult) => r.docId === oldDoc.docId)).toBe(
        false,
      ); // Old doc should not be in results
    });
  });
    ...
    describe('Version Finalization (finalizeVersion)', () => {
      test('should finalize a temporary version and mark it as new', () => {
        ...
        db.insertChunkBatch({
          ...
          docId: docId,
          metas: [
            { pointId, chunkIndex: 0, contentHash: makeDocId(docContent) },
          ],
      ...
      test('should deduplicate versions if content is identical', () => {
        ...
        const initialVersion = db.createVersion(
          collectionId,
          'initial_v',
          'Initial version for dedup',
        );
        const doc1_initial = db.createDoc(
        ...
        const doc2_initial = db.createDoc(
          initialVersion.versionId,
          collectionId,
          'doc2',
          docContent2,
        ...
        db.insertChunkBatch({
          ...
          metas: [
            {
              pointId: makePointId(doc1_initial.docId, 0),
              chunkIndex: 0,
              contentHash: makeDocId(docContent1),
            },
          ],
          texts: [
            {
        ...
        db.insertChunkBatch({
          ...
          metas: [
            {
              pointId: makePointId(doc2_initial.docId, 0),
              chunkIndex: 0,
              contentHash: makeDocId(docContent2),
            },
          ],
          texts: [
            {
        ...
        const { finalVersionId: canonicalVersionId, isNew: isNewCanonical } =
          db.finalizeVersion(initialVersion.versionId);
        expect(isNewCanonical).toBe(true);
        expect(db.getVersion(canonicalVersionId)).toBeDefined();

        // Second version with identical content
        const duplicateVersion = db.createVersion(
          collectionId,
          'duplicate_v',
          'Duplicate version for dedup',
        );
        const doc1_duplicate = db.createDoc(
        ...
        const doc2_duplicate = db.createDoc(
          duplicateVersion.versionId,
          collectionId,
          'doc2',
          docContent2,
        ...
        db.insertChunkBatch({
          ...
          metas: [
            {
              pointId: makePointId(doc1_duplicate.docId, 0),
              chunkIndex: 0,
              contentHash: makeDocId(docContent1),
            },
          ],
        ...
        db.insertChunkBatch({
          ...
          metas: [
            {
              pointId: makePointId(doc2_duplicate.docId, 0),
              chunkIndex: 0,
              contentHash: makeDocId(docContent2),
            },
          ],
        ...
        const {
          finalVersionId: deduplicatedVersionId,
          isNew: isNewDeduplicated,
        } = db.finalizeVersion(duplicateVersion.versionId);
        ...
        expect(
          canonicalVersionDocs.some((d) => d.docId === doc1_initial.docId),
        ).toBe(true);
        expect(
          canonicalVersionDocs.some((d) => d.docId === doc2_initial.docId),
        ).toBe(true);
        // When content is identical, docId will be the same, so the original docId should be present
        expect(
          canonicalVersionDocs.some(
            (d: Doc) => d.docId === doc1_duplicate.docId,
          ),
        ).toBe(true); // This should be true, as doc1_duplicate.docId will be the same as doc1_initial.docId

        const canonicalVersionChunks =
          db.getChunkMetasByVersion(canonicalVersionId);
        expect(canonicalVersionChunks.length).toBe(2); // Only two unique chunks
      });

      test('should handle empty version during finalization', () => {
        const emptyVersion = db.createVersion(
          collectionId,
          'empty_v',
          'Empty Version',
        );
        const { finalVersionId, isNew } = db.finalizeVersion(
      ...
      test('should not finalize if version does not exist', () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          expect.stringContaining('no such versionId'),
        ); // The method does not check for version existence before proceeding to get docIds
