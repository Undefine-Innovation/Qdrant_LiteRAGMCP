...
- **数据流**：
  ```
  SearchService -> SearchCoordinator -> CompositeRetriever
                                       -> [SemanticRetriever, KeywordRetriever, GraphRetriever]
                                       -> [QdrantRepo, SQLiteRepo, GraphRepo]
                                       -> RetrievalResult[] -> FusionStrategy -> DeduplicationStrategy
                                       -> UnifiedSearchResult[] -> SearchService
  ```
- **多数据源集成**：
