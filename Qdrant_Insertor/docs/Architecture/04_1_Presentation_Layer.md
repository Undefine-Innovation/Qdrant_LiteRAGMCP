...
- **文件上传与管理**：提供用户上传文档、批量上传文档、查看文档列表、删除文档等功能。
- **批量操作管理**：提供批量上传、批量删除等操作界面，支持进度监控和结果查看。
- **文档预览与下载**：提供文档预览、多格式下载和缩略图展示功能。
- **搜索结果展示**：将后端返回的检索结果以友好的方式呈现给用户，支持分页浏览。
- **搜索限速与历史**：实现搜索请求限速和历史记录管理，提升用户体验。
...
- **对内接口（与 API 层交互）**：
  ...
  - **主要交互流程**：
    ...
    - **批量文件上传**：`POST /upload/batch` (multipart/form-data)
    - **批量文档删除**：`DELETE /docs/batch`
    - **批量集合删除**：`DELETE /collections/batch`
    - **批量操作进度查询**：`GET /batch/progress/:operationId`
    - **文档预览**：`GET /docs/:id/preview`
    - **文档下载**：`GET /docs/:id/download`
    - **文档缩略图**：`GET /docs/:id/thumbnail`
    - **文档删除**：`DELETE /doc/:id`
    - **查询文档 Chunk 列表**：`GET /doc/:id/chunks`
    - **向量检索**：`GET /search?q=...`
    - **分页向量检索**：`GET /search/paginated`
    - **健康检查**：`GET /healthz`
