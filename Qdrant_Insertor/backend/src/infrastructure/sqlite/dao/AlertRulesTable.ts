...
export class AlertRulesTable {
   ...
   * 更新告警规则
   */
  update(id: string, updates: Partial<Omit<AlertRule, 'id' | 'createdAt'>>): boolean {
    const fields = [];
    const values = [];
  ...
  getAll(activeOnly?: boolean): AlertRule[] {
    ...
    if (activeOnly) {
      query = 'SELECT * FROM alert_rules WHERE is_active = 1 ORDER BY created_at DESC';
    }
  ...
  exists(id: string): boolean {
    const stmt = this.db.prepare('SELECT 1 FROM alert_rules WHERE id = ? LIMIT 1');
  ...
  getStats(): {
    ...
    const totalStmt = this.db.prepare('SELECT COUNT(*) as count FROM alert_rules');
    const totalResult = totalStmt.get() as { count: number };

    const activeStmt = this.db.prepare('SELECT COUNT(*) as count FROM alert_rules WHERE is_active = 1');
