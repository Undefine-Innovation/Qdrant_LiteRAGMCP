...
export class SystemHealthTable {
  ...
  upsert(health: Omit<SystemHealth, 'id' | 'createdAt' | 'updatedAt'>): string {
    ...
    // 返回现有记录的ID
    const selectStmt = this.db.prepare('SELECT id FROM system_health WHERE component = ?');
    const result = selectStmt.get(health.component) as { id: string } | undefined;
    return result?.id || '';
  }
  ...
  getByComponent(component: string): SystemHealth | null {
    const stmt = this.db.prepare('SELECT * FROM system_health WHERE component = ?');
   ...
   */
  getAll(): SystemHealth[] {
    const stmt = this.db.prepare('SELECT * FROM system_health ORDER BY component');
  ...
  getByStatus(status: HealthStatus): SystemHealth[] {
    const stmt = this.db.prepare('SELECT * FROM system_health WHERE status = ? ORDER BY component');
  ...
  getOverallHealth(): {
    ...
    const unhealthyComponents = allComponents.filter(c => c.status !== HealthStatus.HEALTHY);
    const healthyComponents = allComponents.filter(c => c.status === HealthStatus.HEALTHY);

    let overallStatus = HealthStatus.HEALTHY;
    
    if (unhealthyComponents.some(c => c.status === HealthStatus.UNHEALTHY)) {
      overallStatus = HealthStatus.UNHEALTHY;
    } else if (unhealthyComponents.some(c => c.status === HealthStatus.DEGRADED)) {
  ...
  deleteByComponent(component: string): boolean {
    const stmt = this.db.prepare('DELETE FROM system_health WHERE component = ?');
   ...
   * 获取组件健康状态历史统计
   */
  getComponentStats(component: string, days: number = 7): {
    component: string;
    totalChecks: number;
   ...
   * 批量更新多个组件的健康状态
   */
  batchUpdate(healthUpdates: Omit<SystemHealth, 'id' | 'createdAt' | 'updatedAt'>[]): void {
  ...
  componentExists(component: string): boolean {
    const stmt = this.db.prepare('SELECT 1 FROM system_health WHERE component = ? LIMIT 1');
