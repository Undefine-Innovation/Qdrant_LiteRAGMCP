...
export const SyncJobStatsRequestSchema = z.object({
  ...
  status: z.enum(['new', 'split_ok', 'embed_ok', 'synced', 'failed', 'retrying', 'dead']).optional(),
...
export const SystemMetricsRequestSchema = z.object({
  ...
  timeRange: z.enum(['1h', '6h', '24h', '7d', '30d']).optional().default('24h'),
  aggregation: z.enum(['avg', 'min', 'max', 'sum', 'count']).optional().default('avg'),
  component: z.string().optional(),
});
...
export const DashboardDataResponseSchema = z.object({
  ...
  syncJobTrends: z.array(z.object({
    timestamp: z.string(),
    count: z.number(),
    successRate: z.number(),
  })),
...
export const TestNotificationRequestSchema = z.object({
  channelId: z.string(),
  message: z.string().optional().default('Test notification from monitoring system'),
